...
import { getWalletBalance, purchaseAsset, withdrawFunds } from "@/lib/queryFunctions";
...
const PropertyModal = (props: Props) => {
  ...
  const details = {
    ...
    shares: props?.shareAmount?.toString(),
  ...
  const purchaseAssetMutation = useMutation({
    ...
    onError: (error: AxiosError) => {
      ...
      console.error("Asset Purchase Failed:", error.response.data);
    },
  });
  ...
  return (
    <Dialog>
      <DialogTrigger className="flex h-auto w-full items-center justify-center">
        <div
          className=" h-[4rem] w-full rounded-none  flex items-center justify-center   border-2   border-[#4E23CB] bg-transparent  text-2xl font-semibold text-[#4E23CB] hover:bg-[#4E23CB] hover:text-white ">
          Buy
        </div>
      </DialogTrigger>
